...
<body>
  ...
  <?php
    ...
    if($act == 'check') :
      ...
      $sql = "SELECT user_password,login.user_code,user_level, user.dep_id 
        FROM ex_login login
      ...
      if($check_login){
        ...
        $sql = "INSERT INTO ex_log2(log_username,log_host,log_ip,log_status)
            VALUES(?,?,?,?)";
        $stmt = $dbcon->prepare($sql);
        $stmt->execute($data_log);

        if($user_level == 99){
          alertMsg('success','ยินดีต้อนรับ คุณ '.getUserNickname($_SESSION['user_code']).'','manage1.php');
        } else {
          alertMsg('success','ยินดีต้อนรับ คุณ '.getUserNickname($_SESSION['user_code']).'','manage1.php');
        }
